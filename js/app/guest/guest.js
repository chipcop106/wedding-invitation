...
export const guest = (() => {
    ...
    const countDownDate = () => {
        const count = new Date(
            document.body.getAttribute("data-time")
        ).getTime();
    ...
    const open = (button) => {
        ...
        util.changeOpacity(document.getElementById("welcome"), false).then(
            (el) => el.remove()
        );
    };
    ...
    const buildGoogleCalendar = () => {
        ...
        const weddingDate = document.body.getAttribute("data-time");
        const locationAddress = document.body.getAttribute("data-address");
        ...
        const data = new URLSearchParams({
            action: "TEMPLATE",
            text: "Lễ Cưới Của Viết Đạt & Phương Anh",
            dates: `${formatDate(weddingDate)}/${formatDate(weddingDate)}`,
            details:
                "Chúng tôi trân trọng kính mời Quý vị đến tham dự lễ cưới của Viết Đạt & Phương Anh.",
    ...
    const pageLoaded = () => {
        ...
        if (!token || token.length <= 0) {
            ...
            lib.load({
                confetti:
                    document.body.getAttribute("data-confetti") === "true",
            });
        }
